...
import { FileRecord, DialogConfig } from '../types';
...
export const ListView: React.FC<ListViewProps> = ({ 
  files, searchTerm, setSearchTerm, searchDate, setSearchDate, 
  selectedIds, setSelectedIds, setFiles, allFiles, 
  onSearch, openDialog, closeDialog 
}) => {
  ...
  const handleDownloadRequest = () => {
    openDialog({ type: 'download', title: '下載中...', showButton: false });
    setTimeout(() => { 
      openDialog({ type: 'success', title: '下載成功！', showButton: true, onConfirm: closeDialog }); 
    }, 1500);
  };
            ...
            <div className="flex gap-4">
              ...
              <button 
                onClick={() => openDialog({ 
                  ...
                  showButton: true, 
                  onConfirm: () => { 
                    setFiles(allFiles.filter(f => f.id !== file.id)); 
                    closeDialog(); 
                  } 
                })} 
